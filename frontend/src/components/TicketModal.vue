...
import { subscRibeToTickets, fetchTicketMeetingNotes, type ITicket } from '@/api/ticket';
	...
	<VContainer>
		<VCard class="ticket-card">
			...
			<VCardText class="card-section-top">
				...
				<VBtn :onclick="() => meetingNotesVisible = !meetingNotesVisible" class="modal-card-button">
					Show meeting notes</VBtn>
			</VCardText>
			<VDivider></VDivider>
			<VCardText v-if="meetingNotesVisible && meetingNotes.length" v-for="note in meetingNotes"
				class="meeting-note">
				{{ note }}
				<VDivider></VDivider>
			</VCardText>
